...
df = pd.read_excel('MySQL_PYTHON/Data/file_daotao_1.xlsx', sheet_name='Sheet1')
#-----giá trị của các key_value
# value_key = df.iloc[:, :7].values
# print(value_key)
#-----giá trị của file excel
# keys = df.iloc[:, :7].values.flatten().tolist()
# print(excel
#-----giá trị của các row
# 0: mã trường, 1 :tên trường, 2: sở GD&ĐT, 3:Địa chỉ, 4:loại hình, 5 loại trường
#bắt đầu từ hàng 1 có nghĩa là từ hàng 0
#-----các row--------
loai_hinh = df.iloc[:,4]
loai_truong = df.iloc[:,5]
# phong_gd = df.iloc[:,x] không có phòng
ma_truong = df.iloc[:,0].values.flatten().tolist()
ten_truong= df.iloc[:,1].values.flatten().tolist()
so_gddt= df.iloc[:,2]
dia_chi= df.iloc[:,3].values.flatten().tolist()
...
insert_commands = []
query = "SELECT ID FROM PH_GDDT WHERE TEN_PH_GDDT IS NULL" #vì không có phòng nên mặc định là NULL
cursor.execute(query)
result = cursor.fetchone()
phong_gd_id = result['ID']

for i in range(len(ma_truong)):
   insert_command = "INSERT INTO TH_TIN_CHUNG (MaTruong, TenTruong, Dia_Chi, Loai_Hinh_ID, Loai_Truong_ID, Phong_GD_ID) VALUES ('{}', '{}', '{}', {}, {}, {})".format(ma_truong[i], ten_truong[i], dia_chi[i], loai_hinh_id[i], loai_truong_id[i], phong_gd_id)
...
# Đọc file excel và lấy dữ liệu từ sheet có tên "Sheet1"
df = pd.read_excel('/Users/duynguyendinh/Downloads/coderun/python/MySQL_PYTHON/Data/file_daotao_2.xlsx', sheet_name='Sheet1')
#-----giá trị của các key_value
# value_key = df.iloc[4, :7].values
# print(value_key)
#-----giá trị của file excel
# keys = df.iloc[4:, :7].values.flatten().tolist()
# print(excel
#-----giá trị của các row
# 0: mã trường, 1 :tên trường, 2: sở GD&ĐT, 3: Phòng ,4: Địa chỉ, 5:loại hình, 6 loại trường
# Chú ý, bắt đầu từ hàng 6 đổ xuống tức là 5:
#-----các row--------
loai_hinh = df.iloc[5:,5]
loai_truong = df.iloc[5:,6]
phong_gd = df.iloc[5:,3]
ma_truong = df.iloc[5:,0].values.flatten().tolist()
ten_truong= df.iloc[5:,1].values.flatten().tolist()
so_gddt= df.iloc[5:,2]
dia_chi= df.iloc[5:,4].values.flatten().tolist()
...
#========================================GHI FILE 3================================================================
# Đọc file excel và lấy dữ liệu từ sheet có tên "Sheet1"
df = pd.read_excel('/Users/duynguyendinh/Downloads/coderun/python/MySQL_PYTHON/Data/file_daotao_3.xlsx', sheet_name='Sheet1')
#-----giá trị của các key_value
...
#==========================================Ghi file 4===================================================
# Đọc file excel và lấy dữ liệu từ sheet có tên "Sheet1"
df = pd.read_excel('/Users/duynguyendinh/Downloads/coderun/python/MySQL_PYTHON/Data/file_daotao_4.xlsx', sheet_name='Sheet1')
#-----giá trị của các key_value
# value_key = df.iloc[3, :6].values
# print(value_key)
#-----giá trị của file excel
# keys = df.iloc[3:, :7].values.flatten().tolist()
# print(excel
#-----giá trị của các row
# 0: mã trường, 1 :tên trường, 2: sở GD&ĐT, 3:Địa chỉ, 4:loại hình, 5 loại trường
#bắt đầu từ hàng 4 có nghĩa là từ hàng 3
#-----các row--------
...
ten_truong= df.iloc[3:,1].values.flatten().tolist()
so_gddt= df.iloc[3:,2]
dia_chi= df.iloc[3:,3].values.flatten().tolist()
