...
# URL của tập tin JSON chứa danh sách các họ
url = 'https://raw.githubusercontent.com/duyet/vietnamese-namedb/master/uit_member.json'

# Gửi yêu cầu HTTP để lấy dữ liệu từ URL của tập tin JSON chứa danh sách các họ < data của trường uit >
...
# ran, họ tên ViệtNam
def HoVietNam():
    name = random.choice(data)['last_name']
    return name

def TenVietNam():
    name = random.choice(data)['first_name']
    return name
...
def generateDB(database_name): 
    ...
    for i in range(1000000):    # 1mil student 
        ho = HoVietNam() # Fake first name
        ten = TenVietNam() # Fake last name
